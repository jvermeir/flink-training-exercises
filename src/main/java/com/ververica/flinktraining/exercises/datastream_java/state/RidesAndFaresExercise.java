...
public class RidesAndFaresExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends RichCoFlatMapFunction<TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		public void flatMap1(TaxiRide ride, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			if (fare != null) {
				System.out.println(String.format("update map for ride %d with fare for rideId %d", ride.rideId, fare.rideId));
				fareState.clear();
				out.collect(new Tuple2(ride, fare));
			} else {
				System.out.println(String.format("set rides tate for ride %d", ride.rideId));
		...
		public void flatMap2(TaxiFare fare, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			if (ride != null) {
				System.out.println(String.format("update map for fare with rideId %d with ride for rideId %d", fare.rideId, ride.rideId));
				rideState.clear();
				out.collect(new Tuple2(ride, fare));
			} else {
				System.out.println(String.format("set fare state for fare with rideId %d", fare.rideId));
